...
function fpesquisacoordenador_fi($tex,$con){
	...
	if($tex=='')
		...
		$sql = "select e.*, p.nome, 
			...
			from coordenador as e
			inner join pessoal.pessoa as p on p.id=e.pessoa_id 
			where e.ativo='S' and p.nome like '%$tex%' 
			order by p.nome ";
	try {
...
try {
	$sql="SELECT usuario.identificacao AS usuario,usuario.niveldeusuario_id AS nivel, usuario.ativo AS ativo,
...
//die($ssql);
    ...
    $_SESSION['msg']='ERRO PDOException: (leagendaoperador2) ' . $e->getMessage(). ' '. $sql;
	return false;
}	
}	
...
function temagendaservico($spid, $con){
	$ssql="select * from efisio.agenda where servicopessoa_id=$spid";
	$rs= $con->query($ssql);
	if($rs)
		if($rs->rowCount()>0)
			return true;
...
function fleagendaservico($spid, $usu, $grp, $con){
try {	
	if($grp == 'esa'){
	$ssql="SELECT u.id AS usuario, a.*, pe.denominacaocomum AS operador, 
			p.id AS pacienteid, p.denominacaocomum AS paciente, 
			...
			pe.denominacaocomum AS estagiario, pe.nome as estagiarionome,
			es.pessoaid as espessoaid, 
			sp.id as spid
		FROM efisio.agenda AS a
		INNER JOIN efisio.servicopessoa AS sp ON a.servicopessoa_id=sp.id
		INNER JOIN efisio.estagiario AS es ON es.id=a.estagiario_id
		...
		INNER JOIN efisio.statusagenda AS sa ON sa.id=a.statusagenda_id
		INNER JOIN efisio.servico AS s ON sp.servico_id=s.id
		WHERE (sp.id=$spid) AND a.statusagenda_id<3 and u.id=$usu
		ORDER BY a.tipodeagenda_id, a.data DESC, a.horainicial"; 
	}else{	
	$ssql="select a.*, pu.denominacaocomum as operador, 
			p.id as pacienteid, p.denominacaocomum as paciente, 
			ta.descricao as tipodeagenda, sa.descricao as statusagenda,
			s.descricao as nomeservico, s.sigla as servico, 
			es.pessoa_id as espessoaid, pe.denominacaocomum as estagiario, 
			pe.nome as estagiarionome, sp.id as spid
		from efisio.agenda as a
		inner join efisio.servicopessoa as sp on a.servicopessoa_id=sp.id
		inner join efisio.estagiario as es on es.id=a.estagiario_id
		inner join pessoal.pessoa as pe on pe.id=es.pessoa_id
		...
		inner join efisio.statusagenda as sa on sa.id=a.statusagenda_id
		inner join efisio.servico as s on sp.servico_id=s.id
		where (sp.id=$spid) and a.statusagenda_id<3
		order by a.usuario_id, a.tipodeagenda_id, estagiario, a.data desc, a.horainicial"; 
    }
	$rs= $con->query($ssql);
	if($rs)
	   return $rs;
...
try {	
	...
	$ssql="SELECT u.id AS usuario, a.*, pe.denominacaocomum AS operador, 
		...
		ORDER BY sp.id, a.data DESC, a.horainicial"; 
	}else{	
	$ssql="select a.*, pu.denominacaocomum as operador, 
			p.id as pacienteid, p.denominacaocomum as paciente, 
			ta.descricao as tipodeagenda, sa.descricao as statusagenda,
		...
		order by a.usuario_id, sp.id, estagiario, a.data desc, a.horainicial"; 
    }
	$rs= $con->query($ssql);
	if($rs)
	   return $rs;
...
try {	
	$ssql="select t.*, a.statusagenda_id, 
		pp.denominacaocomum as nomesocial, pp.id as epid 
		...
		inner join efisio.servicopessoa as sp on a.servicopessoa_id=sp.id
		inner join efisio.estagiario as ee on ee.id=a.estagiario_id
		inner join pessoal.pessoa as pp on pp.id=ee.pessoa_id
		where sp.id=$spid
		order by t.data desc, t.hora"; 
//$t=ftrace('flehistoricopaciente-->',$ssql);
	$rs= $con->query($ssql);
	if($rs)
	   return $rs;
    else{
...
function leservicopessoaid_fi($spid,$con){
try {
	$sql = "select sp.*, sv.descricao as nomeservico,
			sv.sigla as servico,
			sv.descricao as servicodescricao,
			p.cnpjcpf as cpf, 
			...
			where sp.id=$spid and sp.ativo='S' 
			order by sp.id desc";
...
$ssql = "SELECT qa.*, ppp.denominacaocomum as professorautorizador,
		...
		inner join coordenadordeservico as cs on pu.pessoa_id=cs.pessoa_id and cs.servico_id=sp.servico_id
		where qa.id=$qaid
		order by qa.id";
...
function leusuarioestagiario($eid, $con){
$ssql = "SELECT ee.*, pu.id as eusuarioid
		from efisio.estagiario as ee
		inner join pessoal.usuario as pu on ee.pessoa_id=pu.pessoa_id
		where ee.id=$eid
		order by ee.id";
//$t=ftrace('leusuarioestagiario->',$ssql);		
try {
	$rs= $con->query($ssql);
	if($rs) {
		if($rs->rowCount()>0) {
    ...
    $_SESSION['msg']='ERRO: (leusuarioestagiario) ' . $e->getMessage(). ' '. $ssql;
	return false;
}	

}	

function leassinantetratamentoid($tid,$con){
$ssql = "SELECT t.*, pp.nome as estagiarioassinante,
		pp.denominacaocomum as estagiariored, 
		ee.matricula
		from efisio.tratamento as t
		inner join efisio.estagiario as ee on t.assinaturaestagiario_id=ee.id
		inner join efisio.agenda as ea on t.agenda_id=ea.id
		inner join pessoal.pessoa as pp on ee.pessoa_id=pp.id
		where t.id=$tid
		order by t.id";
//$t=ftrace('leassinantetratamentoaplicadoid--',$ssql);		
try {
	$rs= $con->query($ssql);
	if($rs) {
		if($rs->rowCount()>0) {
    ...
    $_SESSION['msg']='ERRO: (leassinantetratamentoid) ' . $e->getMessage(). ' '. $ssql;
	return false;
}	

}	

function leautorizadortratamentoid($tid,$con){
$ssql = "SELECT t.*, ppp.denominacaocomum as professorred,
		ppp.nome as professorautorizador,
		cs.crefito
		from efisio.tratamento as t
		inner join efisio.agenda as a on a.id=t.agenda_id
		inner join efisio.servicopessoa as sp on sp.id=a.servicopessoa_id
		inner join pessoal.usuario as pu on t.autorizadoprofessor_usuario_id=pu.id
		inner join pessoal.pessoa as ppp on pu.pessoa_id=ppp.id
		inner join coordenadordeservico as cs on pu.pessoa_id=cs.pessoa_id and cs.servico_id=sp.servico_id
		where t.id=$tid
		order by t.id";
//$t=ftrace('leautorizadortratamentoaplicadoid',$ssql);		
try {
	$rs= $con->query($ssql);
	if($rs) {
		if($rs->rowCount()>0) {
    ...
    $_SESSION['msg']='ERRO: (leautorizadortratamentoid) ' . $e->getMessage(). ' '. $ssql;
	return false;
}	
...
function letratamentoid($tid, $con){
$ssql = "SELECT t.*
		from efisio.tratamento as t
		where t.id=$tid
		order by t.id";
try {
    ...
    $_SESSION['msg']='ERRO: (letratamentoid) ' . $e->getMessage(). ' '. $ssql;
	return false;
}	
...
function fezAvaliacao_fi($pid, $spid, $con){
$ssql = "SELECT * FROM questionario AS q
    INNER JOIN questionarioaplicado AS qa ON q.questionarioaplicado_id=qa.id
    INNER JOIN agenda AS a ON qa.agenda_id=a.id
    INNER JOIN servicopessoa AS sp ON a.servicopessoa_id = sp.id
    WHERE (sp.statusservico_id=2 OR sp.statusservico_id=3) 
	AND sp.id=$spid AND sp.pessoa_id=$pid";
try {	
	$rs= $con->query($ssql);
	if($rs) {
		if($rs->rowCount()>0) {
    ...
    $_SESSION['msg']='ERRO PDOException: (fezAvaliacao_fi) ' . $e->getMessage(). ' '. $ssql;
	return ' - ';
}
	
}	

function fezAvaliacao2_fi($agid, $con){
$ssql = "SELECT * FROM questionario AS q
    INNER JOIN questionarioaplicado AS qa ON q.questionarioaplicado_id=qa.id
    INNER JOIN agenda AS a ON qa.agenda_id=a.id
    INNER JOIN servicopessoa AS sp ON a.servicopessoa_id = sp.id
...
$ssql = "SELECT ep.id, pc.denominacaocomum AS professor, 
	...
	sp.statusservico_id, ss.descricao as statuspaciente
	...
	inner join statusservico as ss on sp.statusservico_id=ss.id
	INNER JOIN efisio.servico AS s ON s.id=sp.servico_id
	...
	WHERE (ep.naturezadapessoa_id=2 and (pu.id=$usu and pp.nome like '%$txt%' or $usu=1) and sp.ativo='S')
	ORDER BY professor, statusservico_id, sp.data, paciente, pacienteid";
try {	
	$rs= $con->query($ssql);
	if($rs) {
		return $rs;
...
//$f=ftrace('fpesquisapaciente_fi',$ssql);
	$rs= $con->query($ssql);
	if($rs) {
		return $rs;
	}else{
    ...
    $_SESSION['msg']='ERRO PDOException: (fpesquisapaciente_fi) ' . $e->getMessage(). ' '. $ssql;
	return false;
}	
}	

function fpesquisaagendaservico_fi($tx, $tipoagendaid, $ord, $uid, $pid, $grp, $con){
$tord='';
if($ord=='id'){
	$tord='a.id,';
}	
...
if($grp == 'esa'){
	$ssql="SELECT u.id AS usuario, a.*, pu.denominacaocomum AS operador, 
			...
			p.id as pacienteid, pe.id as epid
		FROM efisio.agenda AS a
		INNER JOIN efisio.servicopessoa AS sp ON a.servicopessoa_id=sp.id
		INNER JOIN efisio.estagiario AS es ON es.id=a.estagiario_id
		inner join pessoal.usuario as u on u.id = a.usuario_id
		...
		inner join efisio.statusservico as ss on ss.id=sp.statusservico_id
		INNER JOIN efisio.servico AS s ON sp.servico_id=s.id
		WHERE (ue.id=$uid) AND  a.tipodeagenda_id=$tipoagendaid and a.statusagenda_id<3 and sp.ativo='S' and ss.id<4
		ORDER BY ".$tord." cliente, a.data DESC, a.horainicial"; 

}else{	
	$ssql="select cs.pessoa_id as cspid, cs.servico_id as cssid,
			a.*, pu.denominacaocomum as operador, p.nome as cliente, 
			ta.descricao2 as tipodeagenda, ss.descricao as statusservico,
			sa.descricao AS statusagenda, pe.id as epid,
			sp.id as sid, sp.statusservico_id,  pe.denominacaocomum AS estagiario,
			s.descricao as servico, s.sigla as siglaservico, 
			p.denominacaocomum as paciente, p.id as pacienteid
		from efisio.coordenadordeservico as cs	
		inner join efisio.servicopessoa as sp on cs.servico_id=sp.servico_id
		...
		inner join efisio.estagiario as es on es.id=a.estagiario_id
		INNER JOIN pessoal.pessoa AS pe ON pe.id=es.pessoa_id
		where (cs.pessoa_id=$pid or $uid=1) AND p.nome LIKE '%$tx%'
...
if($grp == 'esa'){
	...
	$ssql="select a.*, pu.denominacaocomum as operador, p.nome as cliente, 
		...
		inner join efisio.estagiario as es on es.id=a.estagiario_id
		INNER JOIN pessoal.pessoa AS pe ON pe.id=es.pessoa_id
		where (a.usuario_id=$id or $id=1 or '$grp'='coc') AND p.nome LIKE '%$tx%'
		  and a.tipodeagenda_id=$tipoagendaid and a.statusagenda_id<3 and sp.ativo='S' 
		  and ss.id<4
		order by a.data desc, a.horainicial";
}		
...
function fpesquisaservicopessoacadastrada_fi($tex, $sid, $conefi){
	...
	WHERE sp.servico_id=$sid and sp.ativo='S' AND sp.statusservico_id<4 
...
try {
	...
	//$t=ftrace('leusuarioefiporpessoaid',$sql);		
	$rs= $con->query($sql);
	if($rs) {
		if($rs->rowCount()>0) {
			$reg=$rs->fetch(); 
...
function fpesquisapessoaestagiario_fi($textopesquisa,$con){
$tx=trim($textopesquisa);	
try {
	if($tx=='')
	$ssql="SELECT eu.pessoa_id AS pessoaid, eu.ativo, 
			pp.nome, pp.cnpjcpf AS cpf,
			pp.denominacaocomum AS nomesocial, pp.fone,
			pp.situacao_id AS situacaoid, 
			pf.datanascimento 
			FROM efisio.usuario AS eu
			INNER JOIN pessoal.pessoa AS pp ON pp.id=eu.pessoa_id
			INNER JOIN pessoal.pessoafisica AS pf ON pp.id=pf.id
	...
	$ssql="SELECT eu.pessoa_id AS pessoaid, eu.ativo, 
			...
			INNER JOIN pessoal.pessoa AS pp ON pp.id=eu.pessoa_id
			INNER JOIN pessoal.pessoafisica AS pf ON pp.id=pf.id
			WHERE eu.grupo_id=3 and pp.nome like '%$tx%' 
			ORDER BY nome";
    ...
    $_SESSION['msg']='ERRO Exception: (fpesquisapessoaestagiario_fi) ' . $e->getMessage(). ' '. $ssql;
	return false;
}
...
function registraImagem($des, $arq, $path, $plink, $ope, $pid, $con){
	try {
		$dat=formataData(date("Y-m-d H:i"));
		$id=fproximoid("imagem",$con);
		$sql="insert into imagem (id, pessoa_id, data, descricao, 
		nome, path, link, usuario_id) values (
		$id, $pid, '$dat', '$des', '$arq', '$path', '$plink', $ope)
		";
		$ri=$con->query($sql);
		if($ri) return "";
		else return "Erro: ".$sql;
	} catch(PDOException $e) {
		return 'ERRO: (registraImagem) ' . $e->getMessage(). ' '. $sql;
	}		
}	

function enviarImagem($arqdescricao, $error, $size, $name, $tmp_name, $ope,  $pid, $con) {
	if($error) return ("Falha ao enviar imagem!");

	if($size > 3145728) return ("Arquivo muito grande! Máx: 3MB (1024x1024x3)");

	$pasta = dirname(__FILE__) . "/imagens-up/";
	$pasta = str_replace("include","efisio",$pasta);
	$nomeDoArquivo=$name;
	$novoNomeDoArquivo=uniqid();
	$extensao=strtolower(pathinfo($nomeDoArquivo, PATHINFO_EXTENSION));
	$plink="imagens-up/".$novoNomeDoArquivo.".".$extensao;
	if($extensao != "jpg" && $extensao != "png") return ("Apenas arquivos .jpg e .png são aceitos!");

$novoPathName = str_replace("\\","/",$pasta . $novoNomeDoArquivo . "." . $extensao);
	//$novoPathName = $pasta . $novoNomeDoArquivo . "." . $extensao;
		//die($tmp_name.' '.$novoPathName);
	//var_dump($novoPathName); die("");
	$deu_certo = move_uploaded_file($tmp_name, $novoPathName);

	if($deu_certo){
		$ri=registraImagem($arqdescricao, $nomeDoArquivo, $novoPathName, $plink, $ope, $pid, $con);
		return $ri;
	}	
	else
		return "Erro ao fazer uploaded do arquivo de imagem!";
}

function fLeImagensPaciente($pid, $con){
try {	
	$sql = "select pp.denominacaocomum as nomeusuario, 
			pp.id as pid, ei.* 
			from efisio.imagem as ei
			inner join pessoal.usuario as pu on ei.usuario_id=pu.Id	
			inner join pessoal.pessoa as pp on pu.pessoa_id = pp.Id
			where ei.pessoa_id=$pid
			order by ei.id";
	$rs= $con->query($sql); 
	return $rs;
} catch(PDOException $e) {
    $_SESSION['msg']='ERRO pdoexception: (fLeImagemPaciente) ' . $e->getMessage(). ' '. $sql;
	return false;
}		
