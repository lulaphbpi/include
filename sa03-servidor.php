...
function fPrimeiraletramaiusculaX($palavra){
	$n=trim($palavra);
	$nl=strlen($n); 
	//$t=ftrace('fPrimeiraletramaiuscula','<br>Palavra:'.$palavra.' '.$nl.' caracteres.');
	$nn='';
	$i=0;
	if($nl>0){
		$i=1;
		if(ord($n[0])==195){
			$nn=fLetramaiuscula($n[0].$n[1]);
			$i=2;
		}else{
			$nn=fLetramaiuscula($n[0]);
		}	
		//$t=ftrace('fPrimeiraletramaiuscula',' Letra 1:'.$nn);

	}	
	while ($n[$i]<>'$'){
		if(ord($n[$i])==195){
			$m=fLetraminuscula($n[$i].$n[$i+1]);
			$i++;
		}else{
			$m=fLetraminuscula($n[$i]);
		}	
		//$t=ftrace('fPrimeiraletramaiuscula','<br>letra:'.$m);
		//		echo('<br>Letra '.$i.':'.$n[1]);
//		$m=fLetraminuscula($n[$i]);
//		echo(' minuscula '.$i.':'.$m);
		$nn=$nn.$m;
...
function fPrimeiraletradenomemaiuscula($nom){
	$p=fumbrancoapenas($nom);
    $palavras = explode(' ', trim($p));
	$np=count($palavras);
	$n='';
	for($i=0;$i<$np;$i++){
		$lp=strtolower($palavras[$i]);
		//echo('<br>Palavra '.$i.':'.$lp );
		if($lp=='do' or $lp=='dos' 
			or $lp=='da' or $lp=='das' 
			or $lp=='em' or $lp=='de'
			or strlen($lp)==1
			...
			$n.=' '.fPrimeiraletramaiuscula($lp.'$');
	}
	return trim($n);
}

function fPrimeirapalavra($tex){
   $palavras = explode(' ', trim($tex));
   return $palavras[0];
}

function fUltimapalavra($tex, $separador){
	$palavras = explode($separador, trim($tex));
	return end($palavras);
 }
 
 function ncpfToscpf($ncpf){
    // Converte cpf numérico para cpf string de 11 caracteres
    return right('00000000000'.trim(strval($ncpf)),11);
 }

 function fEnesimapalavra($tex, $n, $separador){
	$palavras = explode($separador, trim($tex));
	$np=count($palavras);
	if($n<$np)
	  return $palavras[$n-1];
	else
	  return $palavras[$np-1];
 }

 
 function fValidalista($padrao, $ent, $c){
	// Valida a lista ($ent) separada por um caractere
	// separador ($c) (;, por exemplo), comparando com
	// a Expressão Regular $padrao, retornando V ou F.
	// É útil para validar um texto representando  uma
	// lista telefônica, por exemplo, 
	// "(88)91111-1010;8933331111;990909090".
	// A lista acima tem 3 número de telefones, separados
	// por um ponto-e-vírgula.
	$s=trim($ent);
    $p = explode($c, $s);
	$np=count($p); $ret=false;
	for($i=0;$i<$np;$i++){
	    $st=$p[$i];	
	    if(preg_match($padrao, $st)):
            $ret=true;
		else:
			$ret=false;	
		endif;
 ...
 function fValidaentrada($ent, $tip, $obr){
	// $obr=0, campo não obrigatório
	// <> 1, campo obrigatório
	if(trim($ent)=='')
	    if($obr==0) return true;
	if($tip=='a'){
		if(preg_match(padraoALFA, $ent)){
			return true;
		}else{
			return false;
		}
	...
	if($tip=='n'){
		if(preg_match(padraoALFANUMERO, $ent)){
			return true;
		}else{
			return false;
		}
	...
	if($tip=='1'){
		if(preg_match(padraoNUMERO, $ent)){
			return true;
		}else{
			return false;
		}
	...
	if($tip=='f'){
		if(fValidalista(padraoFONE, $ent, ';'))
		  return true;
		else
		  return false;
	}
...
$lnome=strtolower($nome); $nome=$lnome;
$nnome=str_replace('á', 'a', $nome); $nome=$nnome;
$nnome=str_replace('Á', 'a', $nome); $nome=$nnome;
$nnome=str_replace('ã', 'a', $nome); $nome=$nnome;
$nnome=str_replace('Ã', 'a', $nome); $nome=$nnome;
$nnome=str_replace('â', 'a', $nome); $nome=$nnome;
$nnome=str_replace('Â', 'a', $nome); $nome=$nnome;
$nnome=str_replace('é', 'e', $nome); $nome=$nnome;
$nnome=str_replace('É', 'e', $nome); $nome=$nnome;
$nnome=str_replace('í', 'i', $nome); $nome=$nnome;
$nnome=str_replace('Í', 'i', $nome); $nome=$nnome;
$nnome=str_replace('õ', 'o', $nome); $nome=$nnome;
$nnome=str_replace('Õ', 'o', $nome); $nome=$nnome;
$nnome=str_replace('ú', 'u', $nome); $nome=$nnome;
$nnome=str_replace('Ú', 'u', $nome); $nome=$nnome;
$nnome=str_replace('ç', 'c', $nome); $nome=$nnome;
$nnome=str_replace('Ç', 'c', $nome); $nome=$nnome;
$nnome=str_replace('ñ', 'n', $nome); $nome=$nnome;
$nnome=str_replace('Ñ', 'n', $nome); $nome=$nnome;
$nnome=str_replace('\'', '', $nome); $nome=$nnome;
return $nome;
}

function fumbrancoapenas($tx){
	$tx1=trim($tx);
	$continua=true;
	while ($continua) {
		$continua=false;
...
function fPrimeiraletramaiuscula($tx){
	$t=trim($tx);
